...
- Full-stack roles with a HEAVY, quantified backend requirement — DROP even when React/React Native is present. Drop ONLY when the backend bar is clearly high: a multi-year or expert Node/backend requirement (e.g. "3+ years of Node.js", "X years server-side/backend", "expert/deep Node.js"); OR a heavy database requirement combining depth signals ("strong SQL" plus data modeling / query optimization / database design / DBA-level). These filter the candidate out.
  IMPORTANT: a SOFTER, non-quantified backend expectation is NOT a drop — it is a SCORE CAP (see BACKEND-DEPTH CAP in scoring). Keep it (relevant=true) and score it low. Examples that are CAP-not-drop: "proven experience with Node.js and Express", "solid backend", "strong background in [a backend technology]", "5+ years of full-stack development", "familiarity with databases". And a role that just MENTIONS Node/DB as a light/secondary part ("a plus", "you'll also touch…") is a full fit — keep and score normally.
...
- Low (30-49): React/React Native is core but the overall fit is weaker (remote-anywhere with little Israel tie, thin seniority, OR the backend/full-stack bar is high — see the cap below).
BACKEND-DEPTH CAP (important, applies even when React/RN is clearly present): the candidate is FRONTEND-PRIMARY with LIGHT backend. When a full-stack role expects PROVEN / SOLID / SUBSTANTIAL backend competence as a real requirement — e.g. "proven experience with Node.js and Express", "strong background in [a backend technology]", "5+ years of full-stack/backend development", real API or database work as a core expectation — KEEP the role (relevant=true, do NOT drop it) but CAP the score at ~50 (Low band). It's a stretch fit the candidate could still apply to, so it must stay visible — just ranked low. This holds at ANY seniority, including Staff / Principal / Architect roles: high seniority is NOT a reason to drop (the candidate is senior, 10+ yrs) — it's the backend breadth that caps the score, and as long as React/RN is clearly present (not minor), KEEP it at ~50. The "some Node" bonus and the 70+ bands apply ONLY when backend is genuinely light / secondary / optional — NEVER when proven backend depth is expected. (Reserve DROP for when React/RN is MINOR or ABSENT, or the backend bar is heavy/quantified per the RELEVANCE rules above — 3+ years, expert/deep, or backend-primary.)
